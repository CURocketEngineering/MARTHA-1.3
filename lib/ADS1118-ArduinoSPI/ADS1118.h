...
#define ADS1118_DEFAULT_SPIFREQ (2000000)
/**
...
class ADS1118 {
    public:
        ADS1118();                  ///< Constructor for begin_SPI() style setup.
        ADS1118(uint8_t io_pin_cs); ///< Constructor with default SPI bus.
        ADS1118(uint8_t io_pin_cs, SPIClass *spi);
        void begin();				///< This method initialize the SPI port and the config register
        bool begin_SPI(uint8_t cs_pin, SPIClass *theSPI = &SPI,
                       uint32_t frequency = ADS1118_DEFAULT_SPIFREQ);
	void begin(uint8_t sclk, uint8_t miso, uint8_t mosi);	///< Compatibility helper for Arduino cores with pin-selectable SPI begin().
...
private:
	SPIClass *pSpi = &SPI;
        uint32_t spiFrequency = ADS1118_DEFAULT_SPIFREQ;
	uint8_t lastSensorMode=3;			///< Last sensor mode selected (ADC_MODE or TEMP_MODE or none)
        uint8_t cs = 0xFF;                  ///< Chip select pin (choose one)
